...
# Auto-start Pixcode server in background if not already running.
# This script is sourced from ~/.bashrc on sandbox shell open.

if ! pgrep -f "server/index.js" > /dev/null 2>&1; then
  nohup pixcode start --port 3001 > /tmp/pixcode-ui.log 2>&1 &
  disown

  echo ""
  echo "  Pixcode is starting on port 3001..."
  echo ""
