 ...
 */
export function getAvailableGroups(): import('./agent-manager.js').AvailableGroup[] {
...
async function main(): Promise<void> {
  ...
  if (channels.length === 0) {
    ...
    logger.warn(
      'No messaging channels connected — scheduler + IPC still run (Pixcode embedded mode). Set TELEGRAM_BOT_TOKEN for Telegram channel.',
    );
  }
  ...
  if (channels.length > 0) {
    startMessageLoop().catch((err) => {
      ...
      if (process.env.NANOCLAW_STANDALONE === '1') process.exit(1);
    });
  }
}

/** Start NanoClaw inside Pixcode daemon (no process.exit on missing channels). */
...
export { registerGroup, queue as nanoclawQueue };

// Guard: only run when executed directly, not when imported by tests / Pixcode
const isDirectRun =
  process.env.NANOCLAW_STANDALONE === '1' ||
  (process.argv[1] &&
    new URL(import.meta.url).pathname ===
      new URL(`file://${process.argv[1]}`).pathname);

if (isDirectRun && process.env.NANOCLAW_STANDALONE === '1') {
  main().catch((err) => {
