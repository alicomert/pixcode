...
export const providerMcpService = {
  ...
  async addMcpServerToAllProviders(
    ...
    // Cursor's Windows filter used to live here too; it's been removed so
    // global-MCP writes hit every registered provider uniformly. The UI
    // shows cursor on Windows now — the backend has to match.
